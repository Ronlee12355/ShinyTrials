  ...
  dashboardPage(
    ...
    dashboardBody(
      tabItems(
        tabItem(tabName = 'p1', 
          fluidRow(
            box(title = 'Line plot', status = 'primary', solidHeader = T,plotlyOutput('line_plot')),
        ...
        fluidRow(
          ...
          box(title = 'Select the variable', solidHeader = T,
              selectInput('vari', label = 'Choose the varibale you want', choices = colnames(mtcars)), width = 4, background = "blue")
        )
        ),
        tabItem(tabName = 'p2', 
                fluidRow(
                  tabBox(width = 12,
                         tabPanel('Summary', verbatimTextOutput('sum_iris')),
